    ...
    def get_chunk(self):
        ...
        if self.mode == 0:
            ...
            if self.chunk_id < self.n_chunks:
                ...
                # print(start, end, len(self.X), len(self.y))
        ...
        if self.mode == 1:
            ...
            if self.chunk_id < self.n_chunks:
                ...
                # print(start, end, len(self.X), len(self.y))
                x_to_smote = self.X[start:end]
                y_to_smote = self.y[start:end]

                smote = SMOTE(random_state = self.rstate)
                Xx, yY = smote.fit_resample(x_to_smote, y_to_smote)
                # print(len(Xx), len(yY), str(1))

                self.current_chunk = (Xx, yY)
                return self.current_chunk
            else:
                return None
        if self.mode == 2:
            ...
            if self.chunk_id < self.n_chunks:
                ...
                ada = ADASYN(random_state=self.rstate)
                Xx, yY = ada.fit_resample(x_to_smote, y_to_smote)
                # print(len(Xx), len(yY), str(2))
        ...
        if self.mode == 3:
            ...
            if self.chunk_id < self.n_chunks:
                ...
                ros = RandomOverSampler(random_state=self.rstate)
                Xx, yY = ros.fit_resample(x_to_smote, y_to_smote)
                # print(len(Xx), len(yY), str(3))
